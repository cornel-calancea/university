...
(struct game-state (bird pipes gravity momentum scroll-speed score))
(struct bird (y v-y))
;TODO 8
...
(define (get-initial-state)
  (game-state (bird bird-initial-y 0) (cons (pipe scene-width (+  added-number (random random-threshold))) null)
              initial-gravity initial-momentum initial-scroll-speed 0))
...
; gravitația.
(define (next-state-bird brd gravity)
  (struct-copy bird brd [y (+ (bird-y brd) (bird-v-y brd))] [v-y (+ (bird-v-y brd) gravity)]))
...
(define (next-state-bird-onspace brd momentum)
  (struct-copy bird brd [v-y (- momentum)]))
...
(define (change current-state pressed-key)
  (if (key=? pressed-key " ")
      (struct-copy game-state current-state
                   [bird (next-state-bird-onspace (get-bird current-state) (game-state-momentum current-state))])
      current-state))
...
(define (move-pipes pipes scroll-speed)
                (map
                 (λ (y) (struct-copy pipe y [x (- (pipe-x y) scroll-speed)]))
                   pipes))
...
(define (clean-pipes pipes)
                 (filter (λ (x) (if (> 0 (+ (pipe-x x) pipe-width))
                                                  #f
                                                  #t))
                                       pipes))
...
(define (add-more-pipes pipes)
  (if (< (length pipes) no-pipes)
      (cons (pipe (+ pipe-width pipe-gap (pipe-x (car pipes))) (random random-threshold)) pipes)
      pipes))
...
(define (check-ground-collision bird)
 (if (<= (+ (bird-y bird) bird-height) ground-y)
     #f
     #t))
...
(define (invalid-state? state)
  (or (check-ground-collision (game-state-bird state))
      (check-pipe-collisions (game-state-bird state) (game-state-pipes state))))
...
(define (check-pipe-collisions bird pipes)
  (if (null? pipes)
      #f
      ;verificam coliziunea pentru partea de sus a pipe-ului si partea de jos
   (if (or (check-collision-rectangles (make-posn bird-x (bird-y bird))
                                       (make-posn (+ bird-x bird-width) (+ (bird-y bird) bird-height))
                                       ;limita de sus a pipe-ului de sus e limita de sus frame-ului(0), limita de jos a pipe-ului de sus e gap-y
                                       (make-posn (pipe-x (car pipes)) 0)
                                       (make-posn (+ (pipe-x (car pipes)) pipe-width) (pipe-gap-y (car pipes))))
           (check-collision-rectangles (make-posn bird-x (bird-y bird))
                                       (make-posn (+ bird-x bird-width) (+ (bird-y bird) bird-height))
                                       ;limita de sus a pipe-ului de jos e (pipe-gap-y + gap-height), limita de jos este inaltimea frame-ului
                                       (make-posn (pipe-x (car pipes)) (+ (pipe-gap-y (car pipes)) pipe-self-gap))
                                       (make-posn (+ (pipe-x (car pipes)) pipe-width) scene-height)))
      #t
...
(define (next-state state)
  (struct-copy game-state state [bird (next-state-bird (game-state-bird state) (game-state-gravity state))]
               [pipes (next-state-pipes (game-state-pipes state) (game-state-scroll-speed state))]
               [score (+ 0.1 (game-state-score state))]))
...
(define (draw-frame state)
  (place-image
   ...
   (+ (bird-y (game-state-bird state)) (quotient bird-height 2))
               (place-image
                ground-image
                (quotient scene-width 2)
                (+ ground-y (quotient ground-height 2))
               (place-image
                (score-to-image (game-state-score state))
                text-x
                text-y
                (place-pipes (game-state-pipes state) initial-scene)))))

; Folosind `place-image/place-images` va poziționa pipe-urile pe scenă.
(define (place-pipes pipes scene)
	(if (null? pipes)
            scene
            (let* ((upper-pipe-height (pipe-gap-y (car pipes)))
        (lower-pipe-height (- scene-height (pipe-gap-y (car pipes)) pipe-self-gap))
        (upper-pipe-image (rectangle pipe-width upper-pipe-height "solid" "green"))
        (lower-pipe-image (rectangle pipe-width lower-pipe-height "solid" "green")))
            (place-pipes (cdr pipes) (place-image
                                      upper-pipe-image
                                      (+ (pipe-x (car pipes)) (quotient pipe-width 2))
                                      (quotient upper-pipe-height 2)
                                      (place-image
                                       lower-pipe-image
                                       (+ (pipe-x (car pipes)) (quotient pipe-width 2))
                                       (- scene-height (quotient lower-pipe-height 2))
                                       scene))))))
